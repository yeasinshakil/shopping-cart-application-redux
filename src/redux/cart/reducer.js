    ...
    switch (type) {
        ...
        case ADD_CART:
            if (isExist(state, payload)) {
                ...
                return stateCopy.map((item) => item.id === payload.id ? { ...item, cartQuantity: item.lws_inputQuantity > 0 && item.cartQuantity + 1 , lws_inputQuantity: item.lws_inputQuantity > 0 && item.lws_inputQuantity -1 } : item)
            } else {
                stateCopy.push({ ...payload, cartQuantity: 1, lws_inputQuantity: payload.lws_inputQuantity> 0 && payload.lws_inputQuantity - 1 });
                return stateCopy;
            }
